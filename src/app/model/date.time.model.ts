...
export class DateTimeModel implements NgbDateTimeStruct {
  ...
  public constructor (init?: Partial<DateTimeModel>) {
    // console.log('date-time-model: init: ' + init);
    // console.log(Object.assign(this, init));
    Object.assign(this, init);
  }

  public static fromLocalString(dateString: string): DateTimeModel {
    const date = new Date(dateString);
    console.log('date-time-model: date: ' + date);
    const isValidDate = !isNaN(date.valueOf());
    console.log('date-time-model: date.valueOf(): ' + date.valueOf());
    console.log('date-time-model: isNaN(date.valueOf()): ' + isNaN(date.valueOf()));
    console.log('date-time-model: isValidDate: ' + isValidDate);
  ...
  public toString(): string {
    if (this.isInteger(this.year) && this.isInteger(this.month) && this.isInteger(this.day)) {
      ...
      const isoString = `${pad(year)}-${pad(month)}-${pad(day)} ${pad(hour)}:${pad(minute)}:${pad(second)}`;
      return isoString;
    }
